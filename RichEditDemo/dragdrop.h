...
class CEnumFormatEtcEx : public IEnumFORMATETC
{
public:
	CEnumFormatEtcEx( const vector < FORMATETC >& vFormatEtc )
	{
		m_nRefCount = 0;
		m_nIndex = 0;
...
public:
	...
	STDMETHOD(Clone)( IEnumFORMATETC FAR * FAR* ppCloneEnumFormatEtc )
	{
		...
		*ppCloneEnumFormatEtc = new CEnumFormatEtcEx( m_vFormatEtc );
		( (CEnumFormatEtcEx*)*ppCloneEnumFormatEtc )->AddRef();
		( (CEnumFormatEtcEx*)*ppCloneEnumFormatEtc )->m_nIndex = m_nIndex;

		return S_OK;
	}
};

class CDropSourceEx : public IDropSource
{
public:
	CDropSourceEx()
	{
...
{
public:
	CDataObject( CDropSourceEx *pDropSource )
	{
		m_nRefCount = 0;
...
public:
	...
	STDMETHOD(EnumFormatEtc)( DWORD dwDirection, IEnumFORMATETC __RPC_FAR *__RPC_FAR *ppenumFormatEtc )
	{
		...
		switch ( dwDirection )
		{
		case DATADIR_GET:   *ppenumFormatEtc = new CEnumFormatEtcEx( m_vFormatEtc );
			( (CEnumFormatEtcEx*)*ppenumFormatEtc )->AddRef();
			return S_OK;
...
template < class T >
class CDropTargetExT : public CDropTargetEx
{
public:
	CDropTargetExT( HWND hTargetWnd ) : CDropTargetEx( hTargetWnd )
	{
		m_pDelegate = NULL;
	}
...
template < class T >
class CDataObjectExT : public CDataObject
{
public:
	CDataObjectExT( CDropSourceEx *pDropSource ) : CDataObject( pDropSource )
	{
		m_pDelegate = FALSE;
	}
...
protected:
	CDropSourceEx *m_pDropSource;
	CDataObjectExT< T > *m_pDataObject;
	CDropTargetExT< T > *m_pDropTarget;
	HWND m_hTargetWnd;
...
public:
	BOOL Register( T *pDelegate, BOOL bDropSource = TRUE )
	{
		...
		m_pDropTarget = new CDropTargetExT< T >( m_hTargetWnd );
		...
		m_pDropSource = new CDropSourceEx;
		m_pDropSource->AddRef();

		m_pDataObject = new CDataObjectExT< T >( m_pDropSource );
